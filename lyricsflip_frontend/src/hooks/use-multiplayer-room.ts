...
const mockRoomData: RoomData = {
  ...
  description: "Oorem ipsum dolor sit amet, consectetur adipiscing elit. Nunc vulputate libero et velit interdum,",
  timeLeft: "59:00",
  potWin: "10,000 STRK",
  scores: 456,
  players: [
  ...
  songOptions: [
    ...
    { title: "Blood on The Dance Floor", artist: "ODUMODU BLVCK Bloody Civilian & Wale" },
...
export function useMultiplayerRoom({ roomId, playerName }: UseMultiplayerRoomProps) {
        ...
        switch (data.type) {
          ...
          case "PLAYER_LEFT":
            setRoomData((prevData) => {
              ...
              return {
                ...prevData,
                players: prevData.players.filter((player) => player.id !== data.payload.playerId),
              }
            })
            break
          case "SCORE_UPDATE":
            setRoomData((prevData) => {
              ...
              return {
                ...
                players: prevData.players.map((player) =>
                  player.id === data.payload.playerId ? { ...player, score: data.payload.score } : player,
                ),
